...
def _generate_experiment_result(qlm_result, metadata):
    ...
    #print(dir(qlm_result.qregs[0]))
    #print(qlm_result.qregs[0].length)
    #counts = dict([(hex(s.state.state),np.sqrt(s.probability)) for s in qlm_result.raw_data]) 
    data = ExperimentResultData.from_dict({"counts": counts})


    return ExperimentResult(
        shots=len(qlm_result.raw_data), 
        #shots=qlm_result.meta_data['nbshots'], #MODIFICATION
        success=True,
        data=data,
        header=QobjExperimentHeader(memory_slots=qlm_result.qregs[0].length,metadata=metadata), # MODIFICATION ,added memory slot to inform qiskit on the number of qubits
    )
...
class QPUToBackend(BackendV1):
    ...
    def run(self, run_input, **kwargs):
        ...
        ## MODIFICATION. COMMENTED THE 4 LINES BELOW
        # for res in results:
