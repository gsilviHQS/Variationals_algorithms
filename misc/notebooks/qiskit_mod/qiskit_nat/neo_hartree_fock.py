"""Neo Hartree-Fock initial state."""
...
class NeoHartreeFock(QuantumCircuit):
...
def hartree_fock_bitstring(num_spin_orbitals: int, num_particles: Tuple[int, int]) -> List[bool]:
    """Compute the bitstring representing the Neo Hartree-Fock state for the specified system.
    ...
    quater_orbitals = half_orbitals // 2
    bitstr = np.zeros(num_spin_orbitals, bool)
    bitstr[:num_alpha//2] = True
    bitstr[quater_orbitals : (quater_orbitals + num_beta//2)] = True

    #bitstr[half_orbitals : (half_orbitals + num_alpha//2)] = True
    #bitstr[(half_orbitals + quater_orbitals) : (half_orbitals + quater_orbitals  + num_beta//2)] = True
    print("initial occuption for NEO H2 singlet electron, triplet nuclei state:")
    bitstr[half_orbitals : (half_orbitals + num_alpha)] = True
